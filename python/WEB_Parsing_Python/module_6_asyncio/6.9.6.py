...
class DataDriver:
    def __init__(self):
        self.base_url = "https://parsinger.ru/asyncio/aiofile/3"
        self.all_urls_list = self.get_all_urls()
        self.all_href_urls_list = []
    ...
    async def get_href_urls(self, session, url):
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    if html:
                        soup = BeautifulSoup(html, "lxml")
                        data = soup.find(
                            name="div", attrs={"class": "item_card"}
                        ).find_all(name="a")
                        result = [
                            f"{self.base_url}/depth2/{item.get("href")}"
                            for item in data
                        ]

                        if result:
                            self.all_href_urls_list.extend(result)
        except Exception as e:
            pass

    async def get_all_href_urls(self):
        async with aiohttp.ClientSession() as session:
            tasks = [self.get_href_urls(session, url) for url in self.all_urls_list]

            await asyncio.gather(*tasks)

    def run_get_all_href_urls(self):
        if self.all_urls_list:
            asyncio.run(self.get_all_href_urls())
        else:
    ...
    async def get_and_load_data(self) -> None:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            tasks = [self.process_page(session, url) for url in self.all_href_urls_list]
