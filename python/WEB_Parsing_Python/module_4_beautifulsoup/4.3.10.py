# Цель: Написать код, который будет обрабатывать HTML-структуру, состоящую из различных тегов, включая теги <img>. Код должен находить все теги <img> и суммировать значения их атрибута alt="".
# Код должен:
# Обработать все теги <img> в HTML.
# Извлечь значение атрибута alt="value" для каждого тега <img>. Предполагается, что значения этого атрибута представляют собой целые числа.
...
class DataSoup:
    ...
    def sum_alt(self):
        sum_alt_img = sum(self.alt_list)

        print(f"Сумма всех значений в атрибуте alt тега img: {sum_alt_img}")
