# Цель: Написать код, который будет обрабатывать HTML-структуру, состоящую из тегов <p>. Код должен анализировать текст внутри каждого тега и, если количество символов в тексте (без учета пробелов) является чётным, суммировать значения атрибутов id и class.
#
#
#
# Код должен:
#
# Обработать все теги <p> в HTML.
...
# Выходные данные:
#
# Две переменные: id_sum (сумма значений атрибута id у тегов <p>, удовлетворяющих условиям задачи) и class_sum (аналогично для атрибута class).
# <p id="1" class="2">Текст тега с нечетной длиной</p>
# <p id="3" class="4">Текст</p>
# <p id="5" class="6">Текст тега</p>
# Для данного примера:
#
# Первый тег имеет текст с четной длиной и учитывается.
#
# Второй и третий теги имеют текст с нечётной длиной и не учитываются.
#
# id="1" + class="2" = 3
#
# Сумма ID и CLASS тегов <p> с чётной длиной текста без пробелов: 3
...
class DataSoup:
    ...
    def get_data(self):
        for p in self.p_tags:
            ...
            if len(unspaced_text) % 2 == 0:
                id_value = p.get("id")
                class_value = p.get("class")

                if id_value and id_value.isdigit():
                    self.total_id_sum += int(id_value)

                if (
                    class_value
                    and isinstance(class_value, str)
                    and class_value.isdigit()
                ):
                    class_value = int(class_value)
                    self.total_class_sum += class_value
                elif class_value and isinstance(class_value, list):
                    for value in class_value:
                        if value.isdigit():
                            self.total_class_sum += int(value)

        print(
            f"Сумма ID и CLASS тегов <p> с чётной длиной текста без пробелов: {self.total_id_sum + self.total_class_sum}"
        )
