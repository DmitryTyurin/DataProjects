# Описание задачи: Напишите программу, которая использует пул потоков для анализа текстовых данных.
# Ваша задача: Обработать список предложений, подсчитывая частоту каждой буквы в предложениях и выводить результаты в отсортированном порядке в виде словаря.
#
# Сортировка результата: Отсортируйте общий словарь по алфавиту, знаки препинания выводить не нужно.
#
...
class MessagesAnalyzer:
    ...
    def count_letters(self, sentence: str) -> dict:
    ...
    def merge_results(self, local_counts: list) -> None:
        for local_count in local_counts:
            for char, count in local_count.items():
                self.total_count[char] += count

    def run(self):
        with self.executor as executor:
            futures = [
                executor.submit(self.count_letters, message)
            ...
            for future in as_completed(futures):
                ...
                self.merge_results(local_counts)

        return dict(sorted(self.total_count.items()))


analyzer = MessagesAnalyzer(messages)
result = analyzer.run()
print(result)
