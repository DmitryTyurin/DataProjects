# Напишите корутину, которая будет асинхронно добавлять данные для каждого сообщения в файл после указанного ожидания.
...
FILE_NAME = "data_player.txt"


class WriterPlayground:
    def __init__(self, file_name: str):
        self.file_name = file_name

    async def write_to_file(self, player_name: str, message: str, delay: float):
        await asyncio.sleep(delay)

        async with aiofiles.open(self.file_name, "a", encoding="utf-8") as f:
            await f.write(f"{delay:.2f}: {player_name}: {message}\n")

    async def write_to_file_task(self):
        try:
            async with asyncio.TaskGroup() as tg:
                [
                    tg.create_task(self.write_to_file(player_name, message, delay))
                    for player_name, message, delay in input_data
                ]
        except* Exception as e:
            [logging.warning(error) for error in e.exceptions]

    def taskflow(self):
        asyncio.run(self.write_to_file_task())


writer_playground = WriterPlayground(FILE_NAME)
writer_playground.taskflow()
