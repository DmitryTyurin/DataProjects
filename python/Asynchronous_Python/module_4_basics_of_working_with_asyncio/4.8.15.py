...
# Если для какого-либо участника "Срок доступа" указан None т.е. отсутствует, необходимо возбудить исключение и отменить выполнение всех оставшихся проверок, так как вся секретная операция становится невозможной.
# Важно корректно обрабатывать исключения, чтобы при обнаружении первого участника без допуска, программа не продолжала бессмысленно проверять остальных.
...
# Инициализация анкет: Используйте список словарей data для получения данных участников.
# Создание асинхронных задач: Для каждой анкеты создайте асинхронную задачу, которая проверяет наличие срока доступа, имитируя процесс проверки через asyncio.sleep(), основанный на "Уровень секретности".
# Присвоение имени: Присвойте каждой задаче имя с помощью аргумента name, в функции asyncio.create_task()
# name=f'{participant["Имя"]} {participant["Фамилия"]}'
# Обработка результатов: Используйте await asyncio.wait(tasks, return_when=asyncio.FIRST_EXCEPTION) для ожидания выполнения задач и обработки исключений.
# Если участник имеет действующий доступ, выводить сообщение о подтверждении его участия и продолжительности доступа.
# В случае истекшего или не указанного доступа, выводить ошибку и отменять все остальные проверки.
# # Ошибка доступа возбуждённая через raise
# Ошибка доступа: У участника Michael Strickland срок доступа истек или не указан.
#
...
async def check_access(participant: dict) -> None:
    name = f'{participant["Имя"]} {participant["Фамилия"]}'
    access = participant["Срок доступа"]

    await asyncio.sleep(participant["Уровень секретности"])

    if access is None:
        raise ValueError(
            f"Ошибка доступа: У участника {name} срок доступа истек или не указан."
        )
    else:
        print(
            f"Участник {name} имеет действующий доступ. Продолжительность доступа: {access}"
        )
