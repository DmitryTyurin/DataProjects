# Вашей задачей является обзвон потенциальных клиентов и предложение им ваших услуг.
# Однако, некоторые компании могут быть очень заняты, и время ответа на звонок может варьироваться.
# Ваша задача - написать код, который будет автоматически звонить каждой компании из списка.
# Если время ответа на звонок превышает 5 секунд, звонок должен быть отменен.
...
async def call_company(company_info: dict) -> None:
    company_name = company_info.get("Name")
    phone = company_info.get("Phone")
    call_time = company_info.get("call_time")
    ...
    try:
        ...
        print(f"Company {company_name}: {phone} дозвон успешен")
